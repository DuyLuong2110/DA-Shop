...
namespace ADLVstore.Models.BUS
{
    ...
    {
        public static void ThemOrder(string nguoinhan, string sdt, string diachi, string ShipEmail, string mataikhoan)
        {
        ...
        public static int TongTien(string mataikhoan)
        {
            ...
            {
                return db.Query<int>("select top 1 TongTien from HoaDon where NguoiDat = '" + mataikhoan + "' Order by NgayTao Desc ").FirstOrDefault();
            }
        }
     /*   public static int Soluong(string mataikhoan)
        {
            using (var db = new ShopOnlineConnectionDB())
            {
                return db.Query<int>("select top 1 Soluong from Giohang where MaTaiKhoan = '" + mataikhoan + "'").FirstOrDefault();
            }
        }*/
        ...
        public static IEnumerable<HoaDon> DanhSachHoaDon()
        {
            using (var db = new ShopOnlineConnectionDB())
            {
                string a = "select * from HoaDon Order by NgayTao Desc";
                return db.Query<HoaDon>(a);
            }
        }
        public static IEnumerable<HoaDon> ThongTinDonHang(string mataikhoan)
        {
            using (var db = new ShopOnlineConnectionDB())
            {
                string a = "select * from HoaDon where NguoiDat = '" + mataikhoan + "' Order by NgayTao Desc";
                return db.Query<HoaDon>(a);
            }
        }
